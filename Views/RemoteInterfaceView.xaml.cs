using RemoteInterface.ViewModels;
using System;
...
namespace RemoteInterface.Views
{
    ...
    /// </summary>
    public partial class RemoteInterfaceView : Window
    {
        ViewModel vm = new ViewModel();

        public RemoteInterfaceView()
        {
            if (Process.GetProcessesByName("RemoteInterface").Length == 1)
            { 
                Languages.Language.Culture = new CultureInfo(this.vm.model.settings.language);
                DataContext = vm;
                InitializeComponent();
                UpdateSelectedLanguage();
            } else
            {
                MessageBox.Show(Languages.Language.alwaysRunning);
        ...
        private void ChangeLanguage(object sender, RoutedEventArgs e)
        {
            ...
            if (this.vm.model.settings.language != button.Tag.ToString())
            {
                ...
                MessageBox.Show(Languages.Language.languageMessage);
            }
        }
